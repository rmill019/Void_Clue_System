...
public class ClueDatabase : MonoBehaviour
{
    ...
    public ClueInfo GetClueInfo(string clueName)
    {
        ...
        foreach (GameObject clue in cluePrefabs)
        {
            ...
            else if (clueItem.name == clueName || clueItem.ItemName == clueName)
            {
    ...
    public ClueInfo GetClueInfo(int id)
    {
        ClueItem clueItem;
        ...
        foreach (GameObject clue in cluePrefabs)
        {
            ...
            else if (clueItem.ID == id)
            {
                return new ClueInfo(ref clueItem);
            }
        }

        throw new System.ArgumentException("No clue with the name " + id + " is in the clue database.");
    }
    ...
    public GameObject GetCluePrefab(int id)
    {
        ClueItem script;
        foreach (GameObject cluePrefab in cluePrefabs)
        {
            script = cluePrefab.GetComponent<ClueItem>();

            if (script == null)
                throw new System.InvalidOperationException("Prefab named " + cluePrefab.name + " has no ClueItem component.");
            else if (script.ID == id)
                return cluePrefab;
        }

        Debug.LogError("Clue Prefab with id " + id + " was not found.");
        return null;
    }
